	  ...
	  if (poses.length > 0) {
		...
		skeleton = poses[0].skeleton;
		 ...
		 pose_array.push(pose);
		  ...
		  if(start_flag==false){
				head=(pose.leftEar.x+pose.leftEye.x+pose.nose.x+pose.rightEar.x+pose.rightEye.x)/5;
		  ankle=(pose.rightKnee.x+pose.leftKnee.x)/2;
		 console.log(head,ankle,comp);
		  comp=(head+ankle)/2;
			  start_flag=true;
			}
		  //console.log(pose.leftEar.x,pose.leftEye.x,pose.nose.x,pose.rightEar.x,pose.rightEye.x);
